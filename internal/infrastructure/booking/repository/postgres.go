...
func (r *PostgresRepository) Create(ctx context.Context, b domain.Booking) error {
	_, err := r.db.ExecContext(ctx, `INSERT INTO bookings (id, user_id, room_type_id, check_in, check_out, status, guests, total_price, total_nights) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, b.ID, b.UserID, b.RoomTypeID, b.CheckIn, b.CheckOut, b.Status, b.Guests, b.TotalPrice, b.TotalNights)
	return err
}

func (r *PostgresRepository) FindByID(ctx context.Context, id uuid.UUID) (domain.Booking, error) {
	var b domain.Booking
	err := r.db.GetContext(ctx, &b, `SELECT id, user_id, room_type_id, check_in, check_out, status, guests, total_price, total_nights FROM bookings WHERE id=$1`, id)
	return b, err
}

func (r *PostgresRepository) List(ctx context.Context) ([]domain.Booking, error) {
	var bookings []domain.Booking
	err := r.db.SelectContext(ctx, &bookings, `SELECT id, user_id, room_type_id, check_in, check_out, status, guests, total_price, total_nights FROM bookings ORDER BY created_at DESC`)
	return bookings, err
}
