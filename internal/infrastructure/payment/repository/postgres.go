...
func (r *PostgresRepository) FindByBookingID(ctx context.Context, bookingID uuid.UUID) (domain.Payment, error) {
	var p domain.Payment
	err := r.db.GetContext(ctx, &p, `SELECT id, booking_id, amount, currency, status, provider, payment_url FROM payments WHERE booking_id=$1`, bookingID)
	return p, err
}
